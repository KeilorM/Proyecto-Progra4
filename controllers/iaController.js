        ...
        messages: [
          {
            role: 'system',
            content:
              'Eres un sistema de evaluacion de campamento zombie. Respondes UNICAMENTE con JSON valido, sin markdown ni texto adicional.',
          },
...
export const asignarCargo = async (req, res) => {
  const { nombre, apellidos, habilidades_combate, nivel_confianza, estado_salud } = req.body
  ...
  if (c >= 9 && n >= 9) {
    ...
    cargo_id = 3
    cargo_nombre = 'Explorador'
    categoria = 'COMBATE'
  } else if (c >= 8 && n >= 5 && herido) {
    // herido con alto combate → Técnico mientras se recupera
    cargo_id = 7
    cargo_nombre = 'Técnico de Logística'
    categoria = 'LOGISTICA'
  } else if (c <= 4 && n >= 8) {
    cargo_id = 2
    cargo_nombre = 'Médico'
    ...
    cargo_id = 4
    cargo_nombre = 'Recolector de Agua'
    categoria = 'RECOLECCION'
  }
