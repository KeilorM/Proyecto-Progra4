...
export const completarExploracion = async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE exploracion SET estado = 'COMPLETADA' WHERE id = $1`, [id])

    if (recursos_encontrados && recursos_encontrados.length > 0) {
      const { rows: bodega } = await pool.query(`SELECT id FROM bodega WHERE campamento_id = $1`, [
        campamento_id,
      ])
      ...
      for (const recurso of recursos_encontrados) {
        ...
        await pool.query(
          ...
          [
            ...
            recurso.cantidad,
            req.user.id,
            `Recursos de exploración: ${exploracion[0].nombre_mision}`,
          ]
        )
      }
    }
