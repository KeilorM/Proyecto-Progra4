...
function ModalCrearExploracion({ personas, onClose, onSuccess }: {
  personas: Persona[]
  onClose: () => void
  onSuccess: () => void
}) {
  ...
  const [form, setForm]     = useState({
    nombre_mision: "", fecha_salida: "",
    dias_estimados: 3, dias_extra_max: 1, descripcion_zona: "",
  })
  ...
  return (
    <div style={sharedStyles.modalOverlay} onClick={onClose}>
      <div style={{ ...sharedStyles.modal, maxWidth: "min(560px, 95vw)", maxHeight: "90vh", overflowY: "auto" }}
...
function ModalCompletarExploracion({ exploracion, itemsBodega, onClose, onSuccess }: {
  ...
  return (
    <div style={sharedStyles.modalOverlay} onClick={onClose}>
      <div style={{ ...sharedStyles.modal, maxWidth: "min(500px, 95vw)" }} onClick={e => e.stopPropagation()}>
        <div style={sharedStyles.modalHeader}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {recursos.length === 0 && (
            <div style={{ color: theme.colors.textDim, fontSize: 13, fontFamily: theme.fonts.mono, textAlign: "center", padding: 12 }}>
          ...
          {recursos.map((r, i) => (
            <div key={i} style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <select style={{ ...sharedStyles.input, flex: 2 }}
                ...
                {itemsBodega.map(ib => <option key={ib.id} value={ib.id}>{ib.recurso} ({ib.unidad})</option>)}
              </select>
              ...
              <button type="button" onClick={() => removeRecurso(i)}
                style={{ background: "none", border: "none", color: theme.colors.red, cursor: "pointer", fontSize: 16, padding: "0 4px" }}>
                ✕
              </button>
            </div>
          ))}
...
// ─── MODAL SOLICITUD ─────────────────────────────────────────────────────────
function ModalSolicitud({ campamentos, campamentoActual, onClose, onSuccess }: {
  ...
  return (
    <div style={sharedStyles.modalOverlay} onClick={onClose}>
      <div style={{ ...sharedStyles.modal, maxWidth: "min(480px, 95vw)" }} onClick={e => e.stopPropagation()}>
        <div style={sharedStyles.modalHeader}>
...
export default function ExploracionesPage() {
  ...
  const cargar = async () => {
    try {
      ...
      const [exps, camps, pers, bodega, sols] = await Promise.all([
        getExploraciones(), getCampamentos(), getPersonas(), getBodega(), getSolicitudesEnviadas(),
      ])
  ...
  return (
    <div style={sharedStyles.root}>
        ...
        <div style={{
          ...
          {[
            ...
            <div key={s.label} style={{
              ...
              <div>
                <div style={{ fontSize: isMobile ? 20 : 28, fontWeight: 700, color: s.color, lineHeight: 1 }}>
                  {s.value}
                </div>
                <div style={{ fontSize: 10, color: theme.colors.textDim, letterSpacing: 1, marginTop: 2 }}>
                  {s.label}
                </div>
              </div>
            </div>
                ...
                {exploraciones.map((e, i) => (
                  <div key={e.id} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        <div style={{ color: "#e2e8f0", fontWeight: 600, fontSize: 14 }}>{e.nombre_mision}</div>
                        {e.descripcion_zona && (
                          <div style={{ fontSize: 11, color: theme.colors.textDim, marginTop: 2 }}>{e.descripcion_zona}</div>
                        )}
                      </div>
                      <span style={{
                    ...
                    <div style={{ display: "flex", gap: 16, fontFamily: theme.fonts.mono, fontSize: 11, color: theme.colors.textDim }}>
                ...
                <tbody>
                  {exploraciones.map((e, i) => (
                    <tr key={e.id} style={{ animationDelay: `${i * 0.04}s` }}>
                      ...
                      <td style={{ ...sharedStyles.td, textAlign: "center", color: theme.colors.textDim }}>+{e.dias_extra_max}</td>
                      ...
                      <td style={sharedStyles.td}>
                        <span style={{ fontFamily: theme.fonts.mono, fontSize: 11,
                          color: ESTADO_COLOR[e.estado], border: `1px solid ${ESTADO_COLOR[e.estado]}`, padding: "2px 8px" }}>
                          {e.estado}
                        </span>
                      </td>
                      <td style={sharedStyles.td}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {solicitudes.length === 0 ? (
                  <tr>
                    <td colSpan={isMobile ? 3 : 5} style={{ ...sharedStyles.td, textAlign: "center",
                  ...
                  solicitudes.map(s => (
                    <tr key={s.id}>
                      ...
                      {!isMobile && <td style={sharedStyles.td}>{s.detalle?.descripcion ?? "—"}</td>}
                      <td style={sharedStyles.td}>
                        <span style={{
                          fontFamily: theme.fonts.mono, fontSize: 11,
                          color: s.estado === "PENDIENTE" ? "#facc15" : s.estado === "APROBADA" ? "#4ade80" : "#f87171",
      ...
      {modal === "crear" && (
        <ModalCrearExploracion personas={personas} onClose={() => setModal(null)} onSuccess={cargar} />
      )}
      {modal === "completar" && exploracionActiva && (
        <ModalCompletarExploracion
          exploracion={exploracionActiva} itemsBodega={itemsBodega}
          onClose={() => { setModal(null); setExploracionActiva(null) }} onSuccess={cargar}
        />
      )}
      {modal === "solicitud" && (
        <ModalSolicitud campamentos={campamentos} campamentoActual={campamentoActual}
          onClose={() => setModal(null)} onSuccess={cargar} />
      )}
    </div>
  )
}
