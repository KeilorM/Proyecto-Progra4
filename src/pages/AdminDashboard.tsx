...
function PersonaCard({ p, onEstado }: {
  p: Persona
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ color: "#e2e8f0", fontWeight: 600, fontSize: 14 }}>
            ...
            {p.es_temporal && (
              <span style={{ fontSize: 10, color: "#fb923c", marginLeft: 6, fontFamily: theme.fonts.mono }}>TEMP</span>
            )}
          </div>
          <div style={{ fontFamily: theme.fonts.mono, fontSize: 11, color: theme.colors.textDim, marginTop: 2 }}>
            {p.cargo ?? "Sin cargo"} · #{p.id}
          </div>
        </div>
        <span style={{ fontSize: 11, fontFamily: theme.fonts.mono, color: p.esta_en_campamento ? "#4ade80" : "#94a3b8" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: theme.fonts.mono, fontSize: 9, color: theme.colors.textDim, marginBottom: 3 }}>COMBATE</div>
          <div style={{ display: "flex", gap: 2 }}>
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} style={{ flex: 1, height: 6, background: i < Number(p.habilidades_combate) ? "#f87171" : "rgba(51,65,85,0.4)" }} />
            ))}
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: theme.fonts.mono, fontSize: 9, color: theme.colors.textDim, marginBottom: 3 }}>CONFIANZA</div>
          <div style={{ display: "flex", gap: 2 }}>
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} style={{ flex: 1, height: 6, background: i < Number(p.nivel_confianza) ? "#10b981" : "rgba(51,65,85,0.4)" }} />
            ))}
          </div>
        </div>
        <select
      ...
      <main style={{ ...sharedStyles.main, padding: isMobile ? "16px 12px" : "24px 32px" }}>
        ...
        <div style={{
          ...
          {ESTADOS.map(e => {
            ...
            return (
              <div key={e} style={{
                ...
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: ESTADO_COLOR[e], flexShrink: 0,
                  animation: "pulse 2s ease-in-out infinite" }} />
                <div>
                  <div style={{ fontSize: isMobile ? 22 : 28, fontWeight: 700, color: ESTADO_COLOR[e], lineHeight: 1 }}>
                    {count}
                  </div>
          ...
          <div style={{
            ...
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: theme.colors.red, flexShrink: 0,
              animation: "pulse 2s ease-in-out infinite" }} />
            <div>
              <div style={{ fontSize: isMobile ? 22 : 28, fontWeight: 700, color: theme.colors.red, lineHeight: 1 }}>
                {personas.length}
              </div>
              <div style={{ fontSize: 10, color: theme.colors.textDim, letterSpacing: 1, marginTop: 2 }}>
                Total
          ...
          {(["supervivientes", "solicitudes"] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              ...
              {t === "solicitudes" && solicitudes.filter(s => s.estado === "PENDIENTE").length > 0 && (
                <span style={{ marginRight: 6, background: "#f87171", color: "#fff",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {solicitudes.length === 0 ? (
                  <tr>
                    <td colSpan={isMobile ? 4 : 6} style={{ ...sharedStyles.td, textAlign: "center",
