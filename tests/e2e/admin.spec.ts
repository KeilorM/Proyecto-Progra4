...
test.describe('Admin Dashboard - Acceso y carga', () => {
  ...
  test('Debe mostrar la tabla o lista de personas', async ({ page }) => {
    ...
    const hayContenido = await page
      .locator('text=SANO, text=HERIDO, text=ENFERMO, text=MUERTO')
      .count()
  ...
  test('El botón de agregar persona debe abrir el modal', async ({ page }) => {
    ...
    await expect(
      page.locator('input[placeholder*="ombre"], input[placeholder*="nombre"]').first()
  ...
  test('El modal de agregar persona puede cerrarse', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder*="ombre"]').first()).not.toBeVisible({
      timeout: 3000,
    })
  })
